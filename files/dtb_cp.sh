...
if [ -z "${version}" ]; then
	echo >&2 "dtb_cp.sh: ${DPKG_MAINTSCRIPT_PACKAGE:-kernel package} did not pass a version number"
	exit 2
fi

if [ -f "${source1}" ]; then
	echo "dtb_cp.sh: installing ${source1} to ${target}"
	install -m 644 "${source1}" "${target}"
elif [ -f "${source2}" ]; then
	echo -n "dtb_cp.sh: symlink "
	ln -sfv "$(basename "${source2}")" "${target}"
else
	echo >&2 "dtb_cp: neither ${source1} nor ${source2} not found"
	exit 3
fi
