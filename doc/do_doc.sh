...
main() {
    local img_name img_sha dtb_name dtb_sha uboot_spl_sha uboot_itb_sha
...
render() {
	...
	**$board mmc / emmc / nvme image file**
	 - [$img_name](https://github.com/inindev/$board/releases/download/$tag/$img_name) - complete image of arm64 debian linux $dv built from linux $lv, [patched kernel](https://github.com/inindev/$board/tree/$branch/kernel/patches)
	        \`\`\`sha256: $img_sha\`\`\`
...
parse_sha_file() {
    ...
    uboot_spl_sha="$(cat "$sha_file" | grep 'idbloader\.img$' | head -c64)"
